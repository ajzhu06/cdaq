...
def smart_optimizer(model, name="Adam", lr=0.001, momentum=0.9, decay=1e-5):
    ...
    no_decay_keys = ['s', 'delta_min', 'delta_max', 'alpha', 'beta', 'drift_ema']
    for v in model.modules():
        for p_name, p in v.named_parameters(recurse=0):
            if p_name == "bias":  # bias (no decay)
                ...
                g[1].append(p)
            elif p_name in no_decay_keys:  
                g[1].append(p)  # 加入 g[1]，不进行 decay
                # 可选：打印一下确认加入了
                print(f"Parameter {p_name} set to no-decay.")
            else:
