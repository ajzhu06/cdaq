...
def prepare_model_for_qat(model, device="cpu", args=None):
    ...
    prepare(
        model,
        inplace=True,
        quant_first=args.quant_first,
        quant_last=args.quant_last,
        default_a_bits=args.a_bit,
        default_w_bits=args.w_bit,
        first_a_bits=args.first_a_bit,
        first_w_bits=args.first_w_bit,
        last_a_bits=args.last_a_bit,
        last_w_bits=args.last_w_bit,
        all_positive=True,
...
class DampeningLossScheduler:
    def __init__(self, start_val=0.0, end_val=0.01, start_epoch=0, total_epochs=100):
    ...
    def get_lambda(self, current_epoch):
        ...
        progress = (current_epoch - self.start_epoch) / (self.total_epochs - self.start_epoch)
        progress = max(0.0, min(1.0, progress))


        cos_val = math.cos(math.pi * progress + math.pi)
        factor = (cos_val + 1) / 2

        current_lambda = self.start_val + (self.end_val - self.start_val) * factor
...
def train(hyp, opt, device, callbacks):
    ...
    model = prepare_model_for_qat(model, device=device, args=opt)
    ...
    LOGGER.info("Initializing QAT parameters with one training batch...")
    model.train()
    model.to(device)
    ...
    with torch.no_grad():
        ...
        for imgs, _, _, _ in train_loader:
            ...
            model(imgs)
            break
    LOGGER.info("QAT parameter initialization complete.")
    ...
    ema = None
    damp_scheduler = DampeningLossScheduler(
        start_val=0.0,
        end_val=0.001,
        start_epoch=0,
        total_epochs=epochs,
    ...
    LOGGER.info(f"Initialized Oscillation Dampening Scheduler (Target Lambda: {damp_scheduler.end_val})")
    # Resume
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni == 0:
                ...
                new_learnable_params = [
                    param for name, param in model.named_parameters()
                    if param.requires_grad and id(param) not in existing_param_ids
                ]

                if new_learnable_params:

                    optimizer.param_groups[1]["params"].extend(new_learnable_params)

                    for p in new_learnable_params:
                    ...
                    for name, param in model.named_parameters():
                        if param.requires_grad and id(param) not in existing_param_ids:
                            print(f"[Added to optimizer] {name}: {param.shape}")
            ...
            current_damp_lambda = damp_scheduler.get_lambda(epoch)

            damp_loss_total = torch.tensor(0.0, device=device)
            ...
            for m in actual_model.modules():
                ...
                if isinstance(m, LSQPlusWeightQuantizer):

                    damp_loss_total += m.damp_loss


            if current_damp_lambda > 0 and epoch > 60:

                loss += current_damp_lambda * damp_loss_total
            # Backward
        ...
        if RANK in {-1, 0}:
            ...
            is_last_20 = (epochs - epoch) <= 15

            if not noval or final_epoch or is_last_20:

                do_official_eval = is_coco and is_last_20
...
def parse_opt(known=False):
    ...
    parser.add_argument('--quant_first', action='store_true', help='Enable quantization for the first conv layer')
    parser.add_argument('--no-quant_first', action='store_false', dest='quant_first', help='Disable quantization for the first conv layer')
    parser.set_defaults(quant_first=True)

    parser.add_argument('--quant_last', action='store_true', help='Enable quantization for the last conv layer')
    parser.add_argument('--no-quant_last', action='store_false', dest='quant_last', help='Disable quantization for the last conv layer')
    parser.set_defaults(quant_last=True)
    parser.add_argument('--per_channel', type=bool, default=True)
    parser.add_argument('--first_w_bit', type=int, default=4)
    parser.add_argument('--first_a_bit', type=int, default=4)
    parser.add_argument('--last_w_bit', type=int, default=4)
    parser.add_argument('--last_a_bit', type=int, default=4)
    parser.add_argument('--layerid', type=int, default=0)

    parser.add_argument("--method", type=str, default="cdaq", choices=["cdaq"], help="quantization method")
...
if __name__ == "__main__":
    ...
    opt = parse_opt()
    from quantization.cdaq import prepare, LSQPlusWeightQuantizer
    main(opt)
